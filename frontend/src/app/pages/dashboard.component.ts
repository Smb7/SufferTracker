...
import {
  ...
  pipelineStats, rateNumber, stackBuckets, STATUS_COLORS
} from '../core/chart-data';
import { SankeyChartComponent } from './sankey-chart.component';

@Component({ standalone: true, imports: [NgFor, NgIf, DatePipe, RouterLink, SankeyChartComponent], template: `
  ...
  <ng-container *ngIf="loaded && jobs.length; else noJobs">
    ...
    <section class="panel visualizer-panel" *ngIf="visualizer === 'sankey'"><div class="panel-heading"><div><div class="eyebrow">Flow of the hunt</div><h2>Applied to outcome</h2></div></div><st-sankey [jobs]="jobs" [interviewRounds]="prefs?.interviewRounds ?? 3"></st-sankey><p class="subtle flow-footnote">Link thickness maps to how many applications followed that path. Ghosted and rejected can skip interviews entirely.</p></section>
