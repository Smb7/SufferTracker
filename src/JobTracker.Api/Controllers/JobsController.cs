...
public sealed class JobsController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<JobResponse>> Create(CreateJobRequest request, CancellationToken cancellationToken)
    {
        ...
        var job = new JobApplication
        {
            ...
            Status = request.Status, InterviewRound = request.InterviewRound,
    ...
    public async Task<ActionResult<JobResponse>> Update(Guid id, UpdateJobRequest request, CancellationToken cancellationToken)
    {
        ...
        job.Nickname = request.Nickname?.Trim() ?? string.Empty; job.Status = request.Status; job.InterviewRound = request.InterviewRound;
    ...
    private static JobResponse ToResponse(JobApplication job) => new(job.Id, job.Company, job.Title, job.Description, job.Skills, job.Pay, job.Location, job.Nickname, job.SourceUrl, job.Status, job.InterviewRound, job.AppliedAtUtc, job.UpdatedAtUtc);
}
